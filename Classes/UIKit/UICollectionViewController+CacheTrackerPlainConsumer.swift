    ...
    open func cacheTrackerPlainConsumerEndUpdates() {
        ...
        self.collectionView!.performBatchUpdates({
            for update in updates {
                switch update.type {
                case .itemInsert:
                    self.collectionView!.insertItems(at: [IndexPath(row: update.index, section: 0)])
                case .itemDelete:
                    self.collectionView!.deleteItems(at: [IndexPath(row: update.index, section: 0)])
                case .itemUpdate:
                    self.collectionView!.reloadItems(at: [IndexPath(row: update.index, section: 0)])
                }
            }
        }, completion: nil)
    }
